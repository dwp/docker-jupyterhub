...
RUN apk add --no-cache alpine-sdk bash curl-dev curl g++ gcc gfortran krb5-dev krb5-libs libffi-dev nodejs npm openssl pkgconfig python3 python3-dev py3-pyzmq linux-pam git \
    && apk add --no-cache py-lxml libgfortran py3-numpy py3-numpy-dev py3-scipy py3-scikit-learn jpeg-dev zlib-dev
    
RUN npm install -g configurable-http-proxy

RUN python3 -m ensurepip && \
    pip3 install --upgrade pip setuptools wheel pycurl && \
    pip3 install cvxopt pyvis graphviz pydotplus pytz intervaltree deprecation tqdm stringdist pyemd jsonpickle sympy pulp && \ 
